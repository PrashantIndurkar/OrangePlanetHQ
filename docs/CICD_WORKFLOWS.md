...
This document details the configuration, capabilities, and execution design of the automated integration pipelines and release processes in OrangePlanet.

---

## 🏗️ 1. Continuous Integration Pipeline (`ci.yml`)

The [ci.yml](file:///Users/prashantindurkar/Code/Interviews/Assesment%20Rival/orangeplanet/.github/workflows/ci.yml) workflow runs automatically on every pull request and push to critical branches (`main`, `master`, `dev`). It acts as the primary quality gate ensuring that code builds, lint rules pass, and all database integration tests succeed.
...
services:
  postgres:
    ...
    env:
      ...
      POSTGRES_DB: orangeplanet_test
    ports:
      - 5432:5432
    options: >-
      --health-cmd "pg_isready -U postgres -d orangeplanet_test"
      --health-interval 10s
      --health-timeout 5s
      --health-retries 5
```
...
The [release.yml](file:///Users/prashantindurkar/Code/Interviews/Assesment%20Rival/orangeplanet/.github/workflows/release.yml) workflow is triggered manually via **workflow_dispatch**. It automates semantic versioning upgrades, changelog creation, git tagging, and GitHub Releases.
